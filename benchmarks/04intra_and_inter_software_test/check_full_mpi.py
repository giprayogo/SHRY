...
def get_coordinate_and_symbols(cif_filename, round=4):
    ...
    symbols = [
        str(re.sub("[^a-zA-Z]+", "", i))
        for i in cif_dict["_atom_site_type_symbol"]
    ]
...
def check_structure_consistency(
    str_cif_1, str_cif_2, symmops, write_matched_str=False, round=4
):
    ...
    for iperm, perm in enumerate(perms):
        ...
        # lexsort!! ref1
        lex_perm_coords = np.array(
            [perm_coords[i] for i in np.lexsort(perm_coords.T)]
        )
        lex_perm_symbols = [perm_symbols[i] for i in np.lexsort(perm_coords.T)]

        # lexsort!! ref2
        lex_ref_coords = np.array(
            [ref_coords[i] for i in np.lexsort(ref_coords.T)]
        )
        lex_ref_symbols = [ref_symbols[i] for i in np.lexsort(ref_coords.T)]

        coords_sum = np.sum(
            np.abs(np.array(lex_ref_coords) - np.array(lex_perm_coords))
        )
        coords_sum_each = np.abs(
            [
                np.array(ref_coord) - np.array(perm_coord)
                for ref_coord, perm_coord in zip(
                    lex_ref_coords, lex_perm_coords
                )
            ]
        )
        ...
        if not coords_sum < stol_coord:
            ...
            raise AssertionError(
                f"rank={rank}: Something wrong in the applied symmetry operations!!!"
            )
        ...
        if lex_perm_symbols == lex_ref_symbols and write_matched_str:
            ...
            cif_name = os.path.join(
                atom_1_dirname, f"v_{atom_1_basename}_to_{atom_2_basename}.cif"
            )
...
for row, zipped in enumerate(
    ...
    try:
        ...
        if rank == root:
            ...
            print("\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
            print(
                f"Working on {reference_filename} ({row+1}/{rows})", flush=True
            )
            ...
            reference_structure = LabeledStructure.from_file(
                reference_filename
            )
            ...
            if len(coll_codes) != 1:
                ...
                raise AssertionError(
                    f"rank={rank}: invalid reference CIF filename"
                )
            ...
            for zip_file in zip_files:
                ...
                with zipfile.ZipFile(zip_file, "r") as f:
                    f.extractall(
                        os.path.join(reference_dirname, filebasenozip)
                    )

            # set SHRY and Supercell file names
            supercell_cifs = glob.glob(
                os.path.join(
                    reference_dirname, supercell_cif_glob.format(coll_code)
                )
            )
            shry_cifs = glob.glob(
                os.path.join(
                    reference_dirname, shry_cif_glob.format(coll_code)
                )
            )
            ...
            if len(supercell_cifs) != len(shry_cifs):
                ...
                df.at[row, "Note"] = (
                    df.at[row, "Note"] + ", cif number mismatch"
                )
                raise AssertionError(
                    f"rank={rank}: Mismatch in number of irreducible structure"
                )
        ...
        if check_internal_redundancy:
            ...
            if rank == root:
                print(
                    "Supercell: Check if all the generated structures are symmetrically inequivalent."
                )
            match_flag_list = []
            all_combinations = list(
                itertools.combinations(range(len(supercell_cifs)), 2)
            )
            split_combinations = np.array_split(all_combinations, size)[rank]
            if rank == root:
                print(f"MPI: Num. Core = {size}")
            if rank == root:
                print(f"Total num:{len(all_combinations)}.", flush=True)
                ...
                for np_round in np_round_list:
                    try:
                        match_flag = check_structure_consistency(
                            str_cif_1, str_cif_2, symmops, True, np_round
                        )
                        break
            ...
            comm.Gatherv(
                sendbuf=sendbuf, recvbuf=(recvbuf, sendcounts), root=root
            )
            ...
            if rank == root:
                ...
                if any(match_flag_list):
                    print(
                        f"Supercell, {reference_filename}, the symmetrically-inequivalent test failed."
                    )
                    df.at[row, "Note"] = (
                        df.at[row, "Note"]
                        + "the symmetrically-inequivalent test: supercell failed."
                    )

                else:
                    print(
                        f"Supercell, {reference_filename}, the symmetrically-inequivalent test succeeded."
                    )
        ...
        if check_internal_redundancy:
            ...
            if rank == root:
                print(
                    "SHRY: Check if all the generated structures are symmetrically inequivalent."
                )
            match_flag_list = []
            all_combinations = list(
                itertools.combinations(range(len(shry_cifs)), 2)
            )
            split_combinations = np.array_split(all_combinations, size)[rank]
            if rank == root:
                print(f"MPI: Num. Core = {size}")
            ...
            if rank == root:
                ...
                if any(match_flag_list):
                    print(
                        f"SHRY, {reference_filename}, the symmetrically-inequivalent test failed."
                    )
                    df.at[row, "Note"] = (
                        df.at[row, "Note"]
                        + "the symmetrically-inequivalent test: SHRY failed."
                    )
                    # raise RuntimeError(f"rank={rank}: Failure")
                else:
                    print(
                        f"SHRY, {reference_filename}, the symmetrically-inequivalent test succeeded."
                    )
        ...
        if check_consistency:
            ...
            if rank == root:
                print(
                    "SHRY v.s. Supercell: Check if all the generated structures are consistent between SHRY and Supercell!"
                )
            ...
            if rank == root:
                print(f"MPI: Num. Core = {size}")
            if rank == root:
                print(f"Total num:{len(shry_index_list)}.", flush=True)
            ...
            max_length = np.max(
                [len(l) for l in np.array_split(shry_index_list, size)]
            )
            if len(split_shry_index_list) < max_length:
                for _ in range(max_length - len(split_shry_index_list)):
                    split_shry_index_list = np.append(
                        split_shry_index_list, -1
                    )
            ...
            for i, shry_index in enumerate(split_shry_index_list):
                ...
                if split_inside_loop:
                    ...
                    # print(f":rank={rank}, supercell_match_index={supercell_match_index}",flush=True)
                    supercell_match_index = comm.gather(
                        supercell_match_index, root=root
                    )
                    ...
                    if rank == root:
                        ...
                        pass
                        # print(f"rank={rank}, supercell_index_list={supercell_index_list}",flush=True)
                    supercell_index_list = comm.bcast(
                        supercell_index_list, root=root
                    )
                    # print(f"rank={rank}, supercell_index_list={supercell_index_list}",flush=True)
                    # print(f"rank={rank}, len(supercell_index_list)={len(supercell_index_list)}",flush=True)
            ...
            sendcounts_supercell = np.array(
                comm.gather(len(sendbuf_supercell), root)
            )

            if rank == root:
                recvbuf_shry = np.empty(sum(sendcounts_shry), dtype=int)
                recvbuf_supercell = np.empty(
                    sum(sendcounts_supercell), dtype=int
                )
            else:
                recvbuf_shry = None
            ...
            comm.Gatherv(
                sendbuf=sendbuf_shry,
                recvbuf=(recvbuf_shry, sendcounts_shry),
                root=root,
            )
            comm.Gatherv(
                sendbuf=sendbuf_supercell,
                recvbuf=(recvbuf_supercell, sendcounts_supercell),
                root=root,
            )
            ...
            if rank == root:
                ...
                pairs_pd = pd.DataFrame(
                    np.array(
                        [
                            [shry_cifs[i] for i in recvbuf_shry],
                            [supercell_cifs[i] for i in recvbuf_supercell],
                        ]
                    ).T,
                    columns=["shry_cif", "supercell_cif"],
                )
                pairs_pd.to_csv(
                    os.path.join(
                        reference_dirname, f"{reference_basename}_pairs.csv"
                    )
                )
            ...
            if rank == root:
                ...
                if len(non_match_shry_index_list) > 0:
                    df.at[row, "Note"] = (
                        df.at[row, "Note"]
                        + f"No equiv. str in supercell:{[shry_cifs[i] for i in non_match_shry_index_list]}"
                    )
                    print(
                        "the consistency test was failure. See the xls file."
                    )
                    # raise RuntimeError(f"rank={rank}: Failure")

            comm.Barrier()
            if rank == root:
                print("SHRY v.s. Supercell: the consistency test has done.")
        # fi
...
if rank == root:
    print("\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
    print("Test Done: finalizing MPI...")

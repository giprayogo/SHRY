        ...
        for row, zipped in enumerate(
            ...
            if check_internal_redundancy:
                ...
                for cif_set in tqdm.tqdm(
                    ...
                    **TQDM_CONF,
                    ...
                    match_flag = check_structure_consistency(
                        str_cif_1, str_cif_2, symmops, True
                    )
                ...
                if any(match_flag_list):
                    ...
                    pairs_pd = pd.DataFrame(
                        np.array([match_cif_1, match_cif_2]).T,
                        columns=["supercell1_cif", "supercell2_cif"],
                    )
                    pairs_pd.to_csv(
                        os.path.join(
                            reference_dirname,
                            f"{reference_basename}_supercell_matched.csv",
                        )
                    )
            ...
            if check_consistency:
                ...
                for shry_index in tqdm.tqdm(
                    ...
                    **TQDM_CONF,
                    ...
                    if not match_flag:
                        ...
                        print(
                            "There is no equivalent structure in the supercell outputs!!"
                        )
                ...
                pairs_pd = pd.DataFrame(
                    np.array(
                        [
                            [shry_cifs[i] for i in shry_match_pair_list],
                            [
                                supercell_cifs[i]
                                for i in supercell_match_pair_list
                            ],
                        ]
                    ).T,
                ...
                if (
                    len(non_match_shry_index_list) != 0
                    or len(supercell_index_list) != 0
                ):
                    df.at[row, "Note"] = (
                        df.at[row, "Note"]
                        + f"non_match_shry_cifs: {[shry_cifs[i] for i in non_match_shry_index_list]}, non_match_supercell_cifs:{[supercell_cifs[i] for i in supercell_index_list]}"
                    )

...
def timed_process_run(args, desc=None, average=TIME_LOOP):
    ...
    for i in tqdm(range(average), desc=desc):
        try:
            ...
            if BABEL:
                my_env["BABEL_DATADIR"] = SUPERCELL_PATH
            ...
            with open(
                os.path.join(STDOUT_DIR, f"stdout_{desc}_{i}.out"), "w"
            ) as f:
                f.writelines(p.stdout.decode())
            with open(
                os.path.join(STDERR_DIR, f"stderr_{desc}_{i}.out"), "w"
            ) as f:
            ...
            time_line = [
                ...
                if has_real_time_output(x)
                or has_user_time_output(x)
                or has_sys_time_output(x)
                or has_memory_output(x)
            ]
...
def main():
    ...
    try:
        for row, zipped in enumerate(
            ...
            if SUPERCELL_FLAG:
                try:
                    r, u, s, m, stdout_lines = timed_process_run(
                        supercell_args, desc=f"time_supercell_{cif_basename}"
                    )
                    supercell_substitutions = [
                        x
                        for x in stdout_lines
                        if has_supercell_substitutions(x)
                    ]
                    assert len(supercell_substitutions) == 1
                    try:
                        # supercell_substitutions = int(supercell_substitutions[0].split()[-1])
                        supercell_substitutions = get_supercell_number(
                            supercell_substitutions[0]
                        )
                    except ValueError:
                        # keep as string
                        supercell_substitutions = supercell_substitutions[
                            0
                        ].split()[-1]
                    ...
                    try:
                        ...
                        supercell_equivalent = get_supercell_number(
                            supercell_equivalent[0]
                        )
                    except ValueError:
                        # keep as string
                        supercell_equivalent = supercell_equivalent[0].split()[
                            -1
                        ]
                    ...
                    supercell_substitutions = 0
                    df.at[row, "Note"] = (
                        df.at[row, "Note"] + ", supercell failed"
                    )
                ...
                try:
                    ...
                    # print(e)
                    df.at[row, "Note"] = (
                        df.at[row, "Note"] + ", supercell mismatch"
                    )
            ...
            if SHRY_FLAG:
                try:
                    r, u, s, m, stdout_lines = timed_process_run(
                        shry_args, desc=f"time_shry_{cif_basename}"
                    )
                    ...
                    try:
                        shry_substitutions = get_shry_number(
                            shry_substitutions[0]
                        )
                    except ValueError:
                        # keep as string
                        raise ValueError
